...
interface HistoricalRunSelectorProps {
  ...
  onSelectConversation: (conversation: StoredConversation | null) => void;
}
...
const STORAGE_KEY = 'llama-herd-pinned-historical-runs';
...
const getPinnedRuns = (experimentId?: string): Set<string> => {
  try {
    ...
    console.error('Failed to load pinned historical runs:', e);
  }
  return new Set();
};
...
const savePinnedRuns = (experimentId: string, pinnedIds: Set<string>) => {
  try {
    ...
    console.error('Failed to save pinned historical runs:', e);
  }
};
...
export const HistoricalRunSelector: React.FC<HistoricalRunSelectorProps> = ({
  ...
  const experimentId = conversations[0]?.experiment_id;
  ...
  const getPreviewMessages = (conv: StoredConversation): string => {
