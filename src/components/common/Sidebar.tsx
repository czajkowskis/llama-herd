    ...
    >
      <nav className={`flex flex-col space-y-6 flex-grow`}>
        ...
        >
          ...
          <span className={`font-medium whitespace-nowrap transition-all duration-300 overflow-hidden text-left ${isExpanded ? 'w-40 opacity-100' : 'w-0 opacity-0'}`}>Conversation Viewer</span>
        </NavLink>
        <NavLink 
          to={ROUTES.EXPLORE} 
          className={({ isActive }) => getLinkClass(isActive)} 
