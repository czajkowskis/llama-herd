...
export const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, isExpanded, onMouseEnter, onMouseLeave }) => {
  const linkClass = (pageName: string) => `
    ...
    ${currentPage === pageName ? 'bg-purple-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-700'}
  ...
  return (
    ...
    >
      <nav className={`flex flex-col space-y-6 flex-grow`}>
        <button onClick={() => setCurrentPage('newExperiment')} className={linkClass('newExperiment')} title="New Experiment">
          ...
          <span className={`font-medium whitespace-nowrap transition-all duration-300 overflow-hidden ${isExpanded ? 'max-w-xs opacity-100' : 'max-w-0 opacity-0'}`}>New Experiment</span>
        </button>
        <button onClick={() => setCurrentPage('history')} className={linkClass('history')} title="History">
          ...
          <span className={`font-medium whitespace-nowrap transition-all duration-300 overflow-hidden ${isExpanded ? 'max-w-xs opacity-100' : 'max-w-0 opacity-0'}`}>History</span>
        </button>
      ...
      <div className={`mt-auto flex flex-col space-y-6`}>
        <button onClick={() => setCurrentPage('settings')} className={linkClass('settings')} title="Settings">
          ...
          <span className={`font-medium whitespace-nowrap transition-all duration-300 overflow-hidden ${isExpanded ? 'max-w-xs opacity-100' : 'max-w-0 opacity-0'}`}>Settings</span>
        </button>
        <button onClick={() => setCurrentPage('about')} className={linkClass('about')} title="About LLaMa-Herd">
          ...
          <span className={`font-medium whitespace-nowrap transition-all duration-300 overflow-hidden ${isExpanded ? 'max-w-xs opacity-100' : 'max-w-0 opacity-0'}`}>About</span>
        </button>
      </div>
    </aside>
  );
