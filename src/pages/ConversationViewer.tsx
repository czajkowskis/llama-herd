...
export const ConversationViewer: React.FC = () => {
  ...
  const handleDeleteConversation = async (conversation: StoredConversation) => {
    const index = conversations.findIndex(c => c.id === conversation.id);
    if (index !== -1) {
  ...
  const handleSaveConversationName = async () => {
    if (editingConversationName.trim() && editingConversationId) {
      const updatedConversations = conversations.map((conv) =>
        conv.id === editingConversationId ? { ...conv, title: editingConversationName.trim() } : conv
      );
      ...
      const updatedConversation = updatedConversations.find(c => c.id === editingConversationId);
      if (updatedConversation) {
        try {
          const backendSuccess = await backendStorageService.saveConversation(updatedConversation);
          if (backendSuccess) {
